...
const create = async (req, res) => {
  try {
    ...
    const data = await usersRole.create(params);
...
const get_by_id = async (req, res) => {
  try {
    const data = await usersRole.findByPk(req.params.id);
...
const get_list = async (req, res) => {
  try {
    ...
    Object.keys(params).forEach((key) => {
      if (filter[key]) {
        query.where[key] = filter[key](params[key]);
      }
    });
    ...
    const data = await usersRole.findAndCountAll(query);
